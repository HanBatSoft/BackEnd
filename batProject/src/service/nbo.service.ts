...
import { Nbo_LikesEntity } from 'src/entity/likes.entity';
...
export class NboService {
  constructor(
    ...
    private nboLikesRepository: Repository<Nbo_LikesEntity>,
  ...
  async getSelectNboInfo(
    ...
    try {
      ...
      if (mine) {
        ...
        result = await this.SelectNbo(id, keyword, limit, idx, search, likes);
      }
  ...
  async SelectNbo(
    ...
    try {
      let result = [];
      if (likes) {      
        result = await this.LikesNboList(id,limit)
      } else {        
        result = await this.NboList(id,limit,idx,search,keyword)
      }
      return result;
    } catch (E) {
      console.log(E);
  ...
  async NboList(id:string,limit:number,idx:number,search:string,keyword:string){
    try {
  ...
  async LikesNboList(id:string,limit:number) {
    try {
      const nboSubQuery = await this.likesNboSubQuery();
      const result = await this.nboLikesRepository
        .createQueryBuilder('l')
        .select(
          'a.idx, a.writetime, a.useridx, a.aka, a.likes, a.vilege, a.subject, a.title, a.content, a.isImg, a.commentes, a.img_Idx',
        )
        .innerJoin(nboSubQuery, 'a', 'l.nbo_idx = a.idx')
        .where({ id: id })
        .andWhere({ pause: this.use })
        .orderBy('a.writetime', 'DESC')
        .limit(limit)
        .getRawMany();
        return result;
    } catch (E) {
  ...
  async likesNboSubQuery() {
    const imgSubQuery = await this.nboImgService.JoinImage();
    const subQuery = await this.nboRepository
      .createQueryBuilder()
      .subQuery()
      .select(
        'a.idx, a.writetime, a.useridx, a.aka, a.likes, a.vilege, a.subject, a.title, a.content, a.isImg, a.commentes, b.idx AS img_Idx',
      )
      .from(NboEntity, 'a')
      .leftJoin(imgSubQuery, 'b', 'a.idx = b.nboidx')
      .where(`a.pause = ${this.use}`)
      .getQuery();
    return subQuery;
  }
